...
from Segmentation.unetr_pp.network_architecture.acdc.unetr_pp_acdc import UNETR_PP
...
# 4. Right Kidney
# 5. Bowel
CLASS_LABELS = ["Mean", "Liver", "Spleen", "Left Kidney", "Right Kidney", "Bowel"]
LEARNING_RATE = 1e-4
...
def get_transformer(mode, target_spacing=TARGET_SPACING, clip_range=CLIP_RANGE_DICOM, image_size=IMAGE_SIZE,
    ...
    if mode == "train":
        ...
        return Compose(allways + train + post)
    else:
        return Compose(allways + post)
...
def train_one_epoch(model, optimizer, train_loader, loss_fn, epoch, scaler):
    ...
    return epoch_loss / len(train_loader), iteration
...
def set_up_model_and_train(train_items, val_items, num_epochs, path_save="", cache_path="TMPDIR"):
    ...
    train_transforms = get_transformer(mode="train")
    val_transforms = get_transformer(mode="val")
    ...
    model = UNETR_PP(in_channels=1, out_channels=NUM_CLASSES,image_size=PATCH_SIZE, feature_size=16, num_heads=4, depths=[3, 3, 3, 3], dims=[32, 64, 128, 256],
        do_ds=False
    )
    ...
    path_model = os.path.join(path_save, "best_unetr_plus_plus.pth")
    path_dice_metrics = os.path.join(path_save, "all_unter_plus_plus_dice_metrics.npy")
    path_save = os.path.join(path_save, "save_unter_plus_plus_data.npy")
    try:
    ...
    try:
        ...
        load_array = np.load(path_dice_metrics)
    ...
    scheduler = ReduceLROnPlateau(optimizer=optimizer, mode='max', factor=0.1, patience=25, min_lr=1e-7)
    ...
    for epoch in range(num_epochs):
        ...
        try:
            ...
            np.save(path_save, save_data_np)
        ...
        try:
            ...
            np.save(path_dice_metrics, all_dice_metrics_np)
...
def train(path_root, path_save, num_epochs):
    print(f"Device: {DEVICE}")
    dataset_dict = build_dataset(path_root)
    print(f"Size Training Dataset: {len(dataset_dict)}")
    train_items, val_items, test_items = create_train_test_val_items(dataset_dict)

    all_dice_metrics, model, _ = set_up_model_and_train(train_items, val_items, num_epochs, path_save)
