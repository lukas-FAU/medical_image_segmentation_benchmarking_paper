...
def get_transformer(mode, target_spacing=TARGET_SPACING, clip_range=CLIP_RANGE_DICOM, image_size=IMAGE_SIZE,
    ...
    if mode == "train":
        ...
        return Compose(load_ratic + allways + train + post)
    else:
...
def set_up_model_and_train(train_items, val_items, num_epochs, save_path="", cache_path="TMPDIR"):
    ...
    try:
        state_dict = torch.load("best_swin_unetr_ratic.pth", map_location=DEVICE)
    ...
    all_dice_metrics = []
    try:
        load_array = np.load("all_swin_unetr_dice_metrics.npy")
    ...
    best_val = -1.0
    try:
        load_data = np.load("save_swin_unetr_data.npy")
    ...
    for epoch in range(num_epochs):
        ...
        if val_dice > best_val:
            best_val = val_dice
            torch.save(model.state_dict(), "best_swin_unetr_ratic.pth")
        ...
        try:
            ...
            np.save("save_swin_unetr_data.npy", save_data_np)
        ...
        try:
            ...
            np.save("all_swin_unetr_dice_metrics.npy", all_dice_metrics_np)
