...
from monai.transforms import (LoadImaged, EnsureChannelFirstd, Orientationd, Spacingd, ScaleIntensityRanged,
                              RandFlipd, RandRotate90d, EnsureTyped, ToTensord, Compose, Resized, AsDiscrete,
                              RandScaleIntensityd, RandShiftIntensityd, ClassesToIndicesd,
                              RandCropByLabelClassesd, DeleteItemsd, SpatialPadd)
from monai.data import (PersistentDataset, CacheDataset, Dataset, DataLoader, decollate_batch, pad_list_data_collate)
...
def set_up_model_and_train(train_items, val_items, num_epochs, path_save="", cache_path="TMPDIR"):
    ...
    model = SegResNet(spatial_dims=3, in_channels=1, out_channels=NUM_CLASSES, init_filters=16,
                      dropout_prob=0.2, blocks_down=(1, 2, 2, 4), blocks_up=(1, 1, 1))
    ...
    path_model = os.path.join("", "best_segresnet.pth")
    path_dice_metrics = os.path.join("", "all_segresnet_dice_metrics.npy")
    path_save = os.path.join("", "save_segresnet_data.npy")
    try:
